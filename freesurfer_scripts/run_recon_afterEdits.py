...
#SBATCH -e /scratch/madlab/crash/emu_fs_ae_submit_err
#SBATCH -o /scratch/madlab/crash/emu_fs_ae_submit_out

import os
subjs = ['study_template']
...
for i, sid in enumerate(subjs):
    ...
    with open(script_file, 'wt') as fp:
        ...
        fp.writelines(['#SBATCH -e /scratch/madlab/crash/emu_fs_afteredits_err_%s\n'%sid])
        fp.writelines(['#SBATCH -o /scratch/madlab/crash/emu_fs_afteredits_out_%s\n'%sid])
