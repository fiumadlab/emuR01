...
"""Convert dicom Prisma dirs based on heuristic info
    ...
    for sid in subjs:
        if queue:
            ...
            convertcmd = ' '.join(['python', progname, '-d', dicom_dir_template,
                                   ...
                                   '-z', session, '-c', converter])
            script_file = 'atm-%s_%s.sh' %(sid, session)
            with open(script_file, 'wt') as fp:
                fp.writelines(['#!/bin/bash\n', convertcmd])
                fp.writelines(['#SBATCH --job-name=emuR01_dcm2nii_%s\n'%sid])
                ...
                fp.writelines(['#SBATCH -e /scratch/madlab/crash/emuR01_dcm2nii_err_%s\n'%sid])
                fp.writelines(['#SBATCH -o /scratch/madlab/crash/emuR01_dcm2nii_out_%s\n'%sid])
                fp.writelines([convertcmd])
            outcmd = 'sbatch %s'%script_file
            os.system(outcmd)
            continue

        sdir = dicom_dir_template %(sid, session)
        fl = sorted(glob(sdir))
        #dcmfile = dcm.read_file(fl[0], force=True)
        #print sid, 'Dicom: ', dcmfile.PatientName, sid == dcmfile.PatientName 
        tdir = os.path.join(outdir, sid, session)
...
"""
           Example:
           dicomconvert2_Siemens.py -d /home/data/madlab/dicoms/${TASKNAME}/%s/%s/*/*/*/*/*/* -z s1 OR JustOne -o /home/data/madlab/data/${TASKNAME} -f heuristic_${TASKNAME}.py -s s1 s2 s3
"""))
    ...
    parser.add_argument('-z','--session',dest='session',
                        help='which sesssion were the data collected during')
    ...
    parser.add_argument('-q','--queue',dest='queue',
                        help='LSF partition to use if available')
    ...
    convert_dicoms(args.subjs, os.path.abspath(args.dicom_dir_template),
                   ...
                   converter=args.converter, session=args.session,
                   queue=args.queue)
